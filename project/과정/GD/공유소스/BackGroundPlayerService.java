...
public class BackGroundPlayerService implements Runnable {
	...
	public BackGroundPlayerService(Player player) {
		this.player = player;
		try {
			image = ImageIO.read(new File("image/백그라운드 배경화면(level2).png"));	// image를 읽어온다.
		}catch(Exception e) {
	...
	public void run() {
		// 플레이어의 위치에 따라 색상 확인
		while (true) {
			//플레이어의 왼쪽 위가 탐지 기준이므로 좌표를 (-10,+32), (+64, +32)
			//Color leftColor = new Color(image.getRGB(player.getX()-10, player.getY()+25));
			//Color rightColor = new Color(image.getRGB(player.getX()+50+15, player.getY()+25));
			Color leftColor = new Color(image.getRGB(player.getX()-20, player.getY()+32));
			Color rightColor = new Color(image.getRGB(player.getX()+64+22, player.getY()+32));
			
			//-1 -1 = -2일때 바닥이 흰색
			int bottomColor = image.getRGB(player.getX() + 10, player.getY()+60) //흰색일때 -1
					 +image.getRGB(player.getX() +64 - 10, player.getY()+60);//흰색일때 -1
