...
public class BackGroundPlayerService implements Runnable {
	...
	public BackGroundPlayerService(Player player) {
		this.player = player;
		try {
			image = ImageIO.read(new File("image/test.png")); // image를 읽어온다.
		} catch (Exception e) {
	...
	public void run() {
		...
		while (true) {
			// 플레이어의 왼쪽 위가 (0, 0)
			leftColor = new Color(image.getRGB(player.getX() - 20, player.getY() + 55));
			rightColor = new Color(image.getRGB(player.getX() + 110 + 25, player.getY() + 55));
			topColor = new Color(image.getRGB(player.getX() + 55, player.getY() + 10));

			// -1 -1 = -2일때 바닥이 흰색
			bottomColor = image.getRGB(player.getX() + 10, player.getY() + 100) // 흰색일때 -1
